...
# Visualizations with ggplot2 and base R {.tabset .tabset-pills}
...
# Working with data frames {.tabset .tabset-pills}
...
# Calculating Descriptive Statistics  {.tabset .tabset-pills}
...
# A bit of workflow {.tabset .tabset-pills}
...
# Working with Distributions and random variables {.tabset .tabset-pills}
...
# Hypothesis Tests {.tabset .tabset-pills}
...
# Deviations from Normailty {.tabset .tabset-pills}
...
# Exercises {.tabset .tabset-dropdown}
...
c) It can be seen directly from the visualization of the data that the kne and control group are very similar, whereas the eye group is different from the other two groups. We can also try pairwise t-tests to make this more formal:
...
d) The results do not confirm the original study's claim that light treatment behind the knee leads to changes  phase shift  melatonin production. Experimental design is important, as is replication of scientific findings. In general it is advised to use a double blind study setup, to make sure there are no biases introduced by the experimenter as well as the subjects of the study. 
